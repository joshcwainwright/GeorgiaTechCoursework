...
public class DoublyLinkedList<T> {
    ...
    public void addAtIndex(int index, T data) {
        if (index < 0 || index > size) {
            throw new java.lang.IndexOutOfBoundsException("index does not fall in range: 0<=x<=size");
        }
        if (data == null) {
    ...
    public T removeAtIndex(int index) {
        if (index < 0 || index >= size) {
            throw new java.lang.IndexOutOfBoundsException("index is not in range: 0<=x<size");
        }
        if (index == 0) {
    ...
    public T removeLastOccurrence(T data) {
        ...
        if (size == 0) {
            throw new java.util.NoSuchElementException("data does not exist in doubly linked list");
        }
        if (data == tail.getData() || data.equals(tail.getData())) {
            return removeFromBack();
        } else {
            DoublyLinkedListNode<T> current = tail;
            for (int i = size - 1; i > 0; i--) {
                if (current.getPrevious().getData() == data || data.equals(current.getPrevious().getData())) {
                    if (i == 1) {
                        return removeFromFront();
                    }
