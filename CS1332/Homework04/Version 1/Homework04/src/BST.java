...
public class BST<T extends Comparable<? super T>> {
    ...
    public T remove(T data) {
        ...
        return removeHelper(data, root, true);
    }
     ...
     * @param node BSTNode that holds current node
     * @param first boolean that is only true during the first round of recursion
     * @return T data that was removed
     */
    private T removeHelper(T data, BSTNode<T> node, boolean first) {
        if (node != null) {
            if (data.compareTo(node.getData()) == 0) {
                ...
                if (node.getLeft() == null && node.getRight() == null) {
                    ...
                    removeHelper(node.getRight().getData(), node.getRight(), false);
                } else if (node.getRight() == null) {
                    node.setData(node.getLeft().getData());
                    removeHelper(node.getLeft().getData(), node.getLeft(), false);
                } else {
                    BSTNode<T> suc = findNext(node.getRight());
                    node.setData(suc.getData());
                    removeHelper(suc.getData(), suc, false);
                }
                if (first) {
                    size -= 1;
                }
            ...
            if (data.compareTo(node.getData()) == -1) {
                return removeHelper(data, node.getLeft(), true);
            } else if (data.compareTo(node.getData()) == 1) {
                return removeHelper(data, node.getRight(), true);
            }
        }
