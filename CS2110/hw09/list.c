...
static struct user *create_user(char *name, enum user_type type, union user_data data)
{
    ...
    struct user* newUser;
    if (!(newUser = malloc(sizeof(struct user)))) {
        return NULL;
    }
    if (name == NULL) {
        newUser -> name = NULL;
...
static int user_equal(const struct user *user1, const struct user *user2)
{
    ...
    if (user2 -> name == NULL && user1 -> name != NULL) {
        return 0;
    }
    if (user1 -> name != NULL && user2 -> name != NULL) {
...
struct user_list *create_list(void)
{
    ...
    if (!(newList = malloc(sizeof(struct user_list)))) {
        return NULL;
    }
    newList -> head = NULL;
    newList -> tail = NULL;
...
int push_front(struct user_list *list, char *name, enum user_type type, union user_data data)
{
    ...
    struct user* newUser;
    if (!(newUser = create_user(name, type, data))) {
        return FAILURE;
    }
    if (list -> size == 0) {
        list -> head = newUser;
...
int get_highest_paid(struct user_list *list, struct user **dataOut)
{
    ...
    for (int i = 0; i < list -> size; i++) {
        if (current -> type == INSTRUCTOR) {
            ...
            else if (current -> data.instructor.salary > max -> data.instructor.salary) {
                max = current;
            }
        }
        current = current -> next;
...
int is_passing_all_classes(struct user_list *list, char *name, int *dataOut)
{
    ...
    struct user* current = list -> head;
    for(int i = 0; i < list -> size; i++) {
        if(current -> type == STUDENT && strcmp(current -> name, name) == 0) {
            for(int i = 0; i < current -> data.student.num_classes; i++) {
                if(current -> data.student.grades[i] < 60) {
                    *dataOut = 0;
                    return SUCCESS;
                }
            }
...
int end_semester(struct user_list *list)
{
    ...
    for (int i = 0; i < list -> size; i++) {
        ...
        } else if (current -> type == INSTRUCTOR) {
