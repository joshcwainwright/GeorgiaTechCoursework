...
static metadata_t *find_right(metadata_t *freed_block){
    metadata_t *curr = freed_block -> next;
...
static metadata_t *split_block(metadata_t *block, size_t size) {
    ...
    metadata_t *new = (metadata_t*) ((uint8_t *) block + TOTAL_METADATA_SIZE + block -> size - total);
    block -> size -= total;
    new -> size = size;
    return new;
}
...
static void add_to_addr_list(metadata_t *block) {
    ...
    if (!curr) {
        ...
        if ((uintptr_t) curr > (uintptr_t) block) {
            block -> next = curr;
            address_list = block;
        } else if ((uintptr_t) curr < (uintptr_t) block && !(curr -> next)) {
            curr -> next = block;
        } else {
            while(curr -> next && (uintptr_t) block > (uintptr_t) curr -> next) {
                curr = curr -> next;
            }
...
static metadata_t *find_best_fit(size_t size) {
    ...
    while (curr != NULL) {
        if (curr -> size  == size) {
            return curr;
        } else if (curr -> size > size && (!best || curr -> size < best -> size)) {
            best = curr;
        }
        curr = curr -> next;
    }
...
void my_free(void *ptr) {
    my_malloc_errno = NO_ERROR;
    if (ptr) {
        metadata_t *start = (metadata_t*) ((uint8_t*) ptr - TOTAL_METADATA_SIZE);
        add_to_addr_list(start);

        metadata_t *left = find_left(start);
...
void *my_calloc(size_t nmemb, size_t size) {
    ...
    return memset(new, 0, nmemb * size);
